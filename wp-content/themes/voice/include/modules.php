	...
	function vce_get_module_query( $args = array(), $apply_paged = false ) {
		...
		if ( !empty( $args['manual'] ) ) {
			...
			if ( ( $time_diff = $args['timeto'] ) && $args['time'] ) {
				$q_args['date_query']['before'] = date( 'Y-m-d', strtotime( $time_diff ) );
			}
			if ( ( $time_diff = $args['timeto'] ) && !$args['time'] ) {
				$q_args['date_query'] = array ( 'before' => date( 'Y-m-d', strtotime( $time_diff ) ) );
			}
...
if ( !function_exists( 'vce_allow_onecolumn_module' ) ):
	function vce_allow_onecolumn_module( $mod ) {
		...
		if ( $mod['type'] == 'blank' || ( in_array( $mod['layout'], array( 'c', 'd', 'f' ) ) && in_array( $mod['top_layout'], array( '0', 'c', 'd', 'f' ) ) ) ) {
			return true;
		}
		return false;
